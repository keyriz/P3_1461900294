    ...
    <div class="min-w-screen min-h-screen flex items-center justify-center font-sans overflow-hidden">
        <div class="w-full lg:w-5/6">
            <div class="bg-white shadow-md rounded-lg p-10">
                ...
                <div class="mt-5">
                    <form action="{{ route('jenis_buku.store') }}" method="POST" class="form">
                        @csrf
                        <div class="md:flex flex-row md:space-x-4 w-full text-xs">
                            <div class="mb-3 space-y-2 w-full text-xs">
                                <label class="font-semibold text-gray-600 py-2">Jenis <abbr title="required">*</abbr></label>
                                <input placeholder="Jenis buku..." class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded-md h-10 px-4" required="required" type="text" name="jenis">
                            </div>
                        </div>
                        @if ($errors->any())
