...
@section('title', 'Edit Buku #' . $buku->id)
    ...
    <div class="min-w-screen min-h-screen flex items-center justify-center font-sans overflow-hidden">
        <div class="w-full lg:w-5/6">
            <div class="bg-white shadow-md rounded-lg p-10">
                ...
                <div class="mt-5">
                    <form action="{{ route('buku.update', $buku->id) }}" method="POST" class="form">
                        @csrf
                        @method('PUT')
                        <div class="md:flex flex-row md:space-x-4 w-full text-xs">
                            <div class="mb-3 space-y-2 w-full text-xs">
                                <label class="font-semibold text-gray-600 py-2">Judul <abbr title="required">*</abbr></label>
                                <input placeholder="Judul buku..." class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded-md h-10 px-4" required="required" type="text" name="judul" id="integration_shop_name" value="{{ $buku->judul }}">
                            </div>
                        </div>
                        <div class="mb-3 space-y-2 w-full text-xs">
                            ...
                            <div class="flex flex-wrap items-stretch w-full mb-4 relative">
                                ...
                                <input type="number" class="flex-shrink flex-grow flex-auto leading-normal w-px flex-1 border border-l-0 h-10 border-grey-light rounded-md rounded-l-none px-3 relative focus:border-blue focus:shadow" name="tahun_terbit" required="required" min="1900" max="2099" step="1" value="{{ $buku->tahun_terbit }}">
                            </div>
                        </div>
                        @if ($errors->any())
                        ...
                        <div class="mt-5 text-right md:space-x-3 md:block flex flex-col-reverse">
                            <a href="{{ route('buku.index') }}" class="transition duration-300 ease-in-out mb-2 md:mb-0 bg-white px-5 py-2 text-sm shadow-sm font-medium tracking-wider border text-gray-600 rounded-lg hover:shadow-lg">Batal</a>
